package com.dinfo.autowriting.core.util;
...
public final class ObjectUtil {
    ...
    /**
     * @see ObjectUtil#upperCamelCase(String, String)
     */
    public static String upperCamelCase(String fieldName) {
        return upperCamelCase(null, fieldName);
    }
    ...
    public static String upperCamelCase(String prefix, String fieldName) {
        ...
        if (isLowerCase(fieldChars[0])) {
            ...
            return defaultIfNull(prefix, "") + fieldName;
        }

        char[] prefixChars = defaultIfNull(prefix, "").toCharArray(),
    ...
    /**
     * @see ObjectUtil#lowerCamelCase(String, String)
     */
    public static String lowerCamelCase(String fieldName) {
        return lowerCamelCase(null, fieldName);
    }

    /**
     * The opposite of {@link ObjectUtil#upperCamelCase(String, String)}
     */
    public static String lowerCamelCase(String prefix, String fieldName) {
        if (isEmpty(fieldName))
            return fieldName;

        int fcLen, pcLen;
        char[] fieldChars = fieldName.toCharArray();
        if (isUpperCase(fieldChars[0])) {
            fieldChars[0] += LIMIT_UPPER_AND_LOWER;
        } else {
            return defaultIfNull(prefix, "") + fieldName;
        }
    ...
    public static void checkNonEmpty(String obj, String msg) {
        if (isEmpty(obj))
            throw new NullPointerException(msg + " must be not empty.");
    }

    public static void checkNonEmpty(Object[] obj, String msg) {
        if (isEmpty(obj))
            throw new NullPointerException(msg + " must be not empty.");
    }
