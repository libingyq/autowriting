...
import com.dinfo.fi.service.out.ConceptDataService;
 ...
 */
@RestController
@FuncAttr(name = "根据概念代码获得数据",type = FuncType.ObjectFunc)
public class GrabConceptByCode implements ObjectFuncInterface{

    @Autowired
    private ConceptDataService stockDataService;

    @Override
    public List<DataInfo> queryData(QueryInfoConditionDto query) {
        String code = query.getTip();
        Response stockByCodeAndTimeRange = stockDataService.getConceptByCodeAndTimeRange(code, query.getStartTime(), query.getEndTime());
