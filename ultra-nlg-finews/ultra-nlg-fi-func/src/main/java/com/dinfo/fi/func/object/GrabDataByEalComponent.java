...
public class GrabDataByEalComponent implements ObjectFuncInterface{
    ...
    public List<DataInfo> queryData(QueryInfoConditionDto query) {
        String ealComponent = query.getTip();
        Response stockCodeByEalComponent = stockInfoService.getStockCodeByEalComponent(ealComponent);
        if(stockCodeByEalComponent.isNotSuccess()){
            throw new FuncException("获取股票代码失败，股市为："+ealComponent);
        }
