 ...
 */
@Component
@FuncAttr(name = "根据股票行业获得数据",type = FuncType.ObjectFunc)
public class GrabDataByIndustry implements ObjectFuncInterface{
    ...
    public List<DataInfo> queryData(QueryInfoConditionDto query) {
        String industry = query.getTip();
        Response stockCodeByConcept = stockInfoService.getStockCodeByIndustry(industry);
        if(stockCodeByConcept.isNotSuccess()){
            throw new FuncException("获取股票代码失败，股票概念为："+industry);
        }
